  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: COLORS.cream, color: COLORS.charcoal }}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <a href="#" className={`transition-all duration-300 ${scrolled ? 'opacity-100' : 'opacity-0'}`}>
              <img src="Crunchy.svg" alt="Crunchyology" className="h-12" />
            </a>
            <div className={`hidden md:flex space-x-8 ${scrolled ? 'opacity-100' : 'opacity-0'}`}>
              {NAV_LINKS.map(link => (
                <a key={link.href} href={link.href}
                   className={`text-white font-medium hover:text-golden transition-colors ${activeSection === link.href.slice(1) ? 'text-golden' : ''}`}
      ...
      <section className="relative h-screen overflow-hidden">
        ...
        <div className="absolute inset-x-0 top-1/4 -translate-y-1/2 z-20 flex justify-center" style={{ animation: 'float 6s ease-in-out infinite' }}>
<img src="Crunchy.svg" alt="Crunchyology logo" className="h-[24rem] sm:h-[28rem] md:h-80 lg:h-96 xl:h-[40rem] w-auto drop-shadow-2xl" />        </div>
        
        {/* Centered Navigation Buttons */}
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 w-full max-w-4xl px-4 z-10">
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 md:gap-6">
            {NAV_LINKS.map((btn, index) => (
              <a key={btn.href} href={btn.href}
                className="group backdrop-blur-2xl text-white font-bold px-8 py-5 rounded-full shadow-2xl transition-all duration-500 text-lg md:text-xl flex items-center gap-3 transform hover:scale-105 hover:-translate-y-1 border-2 border-white/20 hover:border-white/40 w-full sm:w-auto justify-center"
                style={{ 
